...
# pattern1 = r"регулярн\w+ выражен\w+"
# print(type(pattern1))
# print(pattern1)
# pattern2 = re.compile(r"регулярн\w+ выражен\w+")
# print(type(pattern2))
# print(pattern2)
# result = re.sub(pattern, "RegEx", text)
# result = pattern2.sub("RegEx", text)
...
# pattern = re.compile(r'(8|\+7)?\s*\((\d+)\)\s*(\d+)[\s-]*(\d{2})[\s-]*(\d{2})')

# result = pattern.findall(text2)
# print(len(result))
# print(result)

# result = pattern.sub(r"+7 (\2) \3-\4-\5", text2)
# print(result)

pattern = re.compile(r"(8|\+7)?\s*\((?P<citycode>\d+)\)\s*(?P<phone1>\d+)[\s-]*(\d{2})[\s-]*(\d{2})")
